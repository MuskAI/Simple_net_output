...
class DataParser():
    def __init__(self, batch_size_train):
        ...
        self.train_image_list, self.gt_list = data_prepare.MixData().gen_dataset()
        print(self.train_image_list)
        print(self.gt_list)
    ...
    def get_batch(self, batch, train=True):
        ...
        for idx, b in enumerate(batch):
            if train:
                ...
                dou_path = os.path.join(self.Y_train[index])
                dou_em = Image.open(dou_path)
                if len(dou_em.split()) ==3:
                    dou_em = np.array(dou_em,'uint8')
                    dou_em = dou_em[:,:,0]

                    dou_em = Image.fromarray(dou_em)
                t = np.array(dou_em,dtype='uint8')
                # 在这里获取8张图，从左上角按照顺时针顺序,返回的是一个长度为8的列表
                try:
                    relation_8_map = gen_8_map(t)
                except:
                    print(t.shape)
                    traceback.print_exc()
                for i in range(8):
