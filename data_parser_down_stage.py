...
from data_prepare import MixData
...
class DataParser():
    def __init__(self, batch_size_train):
        ...
        #
        # self.train_file = '/media/liu/File/10月数据准备/10月12日实验数据/splicing/tamper_result_320'
        # self.double_edge_file = '/media/liu/File/10月数据准备/10月12日实验数据/splicing/tamper_result_320/ground_truth_result'
    ...
    def get_batch(self, batch, train=True):
        ...
        for idx, b in enumerate(batch):
            if train:
                ...
                dou_path = self.Y_train[index]
                dou_em = Image.open(dou_path)
                dou_em_numpy = np.array(dou_em)
                if dou_em_numpy.size > 320*320:
                    dou_em_numpy = dou_em_numpy[:,:,0]
                else:
                    dou_em_numpy = np.array(dou_em_numpy[:, :])
                # 在这里获取8张图，从左上角按照顺时针顺序,返回的是一个长度为8的列表
                relation_8_map = gen_8_map(dou_em_numpy)
                for i in range(8):
                ...
                dou_em = Image.open(self.Y_test[index])
                dou_em = np.array(dou_em)
                plt.imshow(dou_em)
                plt.show()
                print(dou_em.shape)
