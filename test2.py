...
test = glob.glob('/media/liu/File/10月数据准备/10月12日实验数据/CompressedImageCM')
import os
...
if __name__ == "__main__":
    ...
    model.load_weights('/home/liu/libiao/Simple_net_output/checkpoints/10.16_new_out/checkpoint.48-0.0235-0.9988-0.9394-0.9038-0.9179.hdf5',by_name=True)

    for dir_path in test:
        dir_test=os.listdir(dir_path)
        dir = dir_path.split('/')[-1]
        save_dir_path = os.path.join('/media/liu/File/10月数据准备/epoch40', dir)
